...
import com.dystify.marioPartyEvent.Util;
...
public class Player
{
	...
	public Player(String name, String resourceDir, Paint p) {
		...
		setActiveFace(false);
		setPlace(Place.FOURTH);
	}
	
	
	/**
	 * creates a shadow player w/o a scoreboard. Doesn't set spritestate either
	 * @param resourceDir
	 */
	public Player(String resourceDir) {
		this.resourceDir = resourceDir;
		playerSprite = new ImageView();
	}
...
//		System.out.println(Math.toDegrees(ang));
		...
		{	
			Platform.runLater(() -> {
				...
				try {
					...
					System.out.println("in walk(), encountered " +e.getClass().getName());
	...
	public int getNextRoll() {
		Command c = Comm.getInst().waitOnCommand(300000000, CommandType.ROLL);
		return Util.getDiceRoll(c.getResponse());
	}
